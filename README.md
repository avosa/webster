...
```bash
sudo mv wster /usr/local/bin
```

6. Now you can create `a .wb` file containing a simple arithmetic expression, such as `2 + 3 \* 4`, in any directory on your computer. Save it as `app.wb`.
7. Open a terminal and navigate to the directory containing the app.wb file, then run the wster command followed by the `.wb` file you created: `wster app.wb`.
8. This will evaluate the arithmetic expression and print the result, regardless of where the `app.wb` file is located on your computer.
